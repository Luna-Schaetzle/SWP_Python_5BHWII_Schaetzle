...
class Hotel:
    ...
    def process_day(self):
        ...
        # Gästeschlange verarbeiten
        self.process_guest_queue()  # NEU: Gästeschlange verarbeiten
    ...
    def handle_random_events(self):
        ...
        if random.randint(1, 100) <= 20:
            ...
            # NEU: Zusätzliche Ereignisse
                ...
                guest_name = f"Gast_{random.randint(1000,9999)}"
                print(f"\n🚪 Ein neuer Gast namens {guest_name} möchte ein {requested_type}-Zimmer für {requested_nights} Nacht/Nächte buchen.")
                guest = Guest(guest_name, random.randint(18, 75), "Unbekannt")
    ...
    def process_guest_queue(self):  # NEU
        if self.guest_queue:
            guest, room_type, nights = self.guest_queue.pop(0)
            available_rooms = self.get_available_rooms(self.current_day, self.current_day + nights, room_type)
            if available_rooms:
                print(f"\n📋 Gästeschlange: {guest.name} möchte ein {room_type}-Zimmer für {nights} Nacht/Nächte buchen.")
                choice = input("Annehmen? (j/n): ").lower()
                if choice == 'j':
                    booking = self.book_room(available_rooms[0], guest, self.current_day, self.current_day + nights)
                    if booking:
                        print("✅ Buchung angenommen!")
                        # Optionale: Feedback basierend auf Zimmerart
                        self.reputation += 1
                else:
                    print(f"❌ Buchungsanfrage von {guest.name} abgelehnt.")
                    # Optionale: Rufverlust bei Ablehnung
                    self.reputation -= 1
            else:
                print(f"\n📋 Gästeschlange: {guest.name} möchte ein {room_type}-Zimmer, aber kein Zimmer ist frei. Buchung abgelehnt.")
                self.reputation -= 2  # Rufverlust bei Ablehnung ohne Grund
